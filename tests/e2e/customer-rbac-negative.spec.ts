...
test("signed-in role without customer permissions fails closed on all six surfaces", async ({
  page,
}, testInfo) => {
  const routes = [
    "/en/dashboard/customers",
    "/en/dashboard/customers/new",
    "/en/dashboard/customers/" + primaryCustomerId,
    "/en/dashboard/customers/" + primaryCustomerId + "/edit",
    "/en/dashboard/customers/" + primaryCustomerId + "/orders",
  ...
  for (const route of routes) {
    ...
    await expect(page.locator("body")).not.toContainText(primaryCustomerName);
  }
  ...
  const screenshotPath = join(
    evidenceDir,
    "denied-all-surfaces-en-desktop.png",
  );
