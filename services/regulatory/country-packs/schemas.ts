...
const eInvoicingDocumentTypeSchema = z.enum(["POS_RECEIPT", "SALES_INVOICE", "CREDIT_NOTE"])

const eInvoicingRequiredFieldSchema = z.object({
  scope: z.enum(["SELLER", "BUYER", "DOCUMENT", "LINE", "TAX", "PAYMENT", "SOURCE_TRACE"]),
...
const eInvoicingAuthorityChannelSchema = z.object({
  ...
  channelType: z.enum(["API", "PORTAL", "FILE_UPLOAD", "EMAIL", "IN_PERSON", "UNKNOWN"]),
  ...
  credentialStoragePolicy: z.enum(["VAULT_ONLY", "NOT_APPLICABLE", "REQUIRES_CONFIGURATION"]),
...
const eInvoicingMetadataSchema = z.object({
    ...
    .array(
      regulatoryEnvelopeSchema.extend({
        value: z.object({
          ...
          missingFieldPolicy: z.enum(["BLOCK_CERTIFICATION", "ALLOW_DRAFT_ONLY", "REQUIRES_EXPERT_REVIEW"]),
        }),
      }),
    )
    .min(1),
  certificationPolicy: z
    .array(
      regulatoryEnvelopeSchema.extend({
        value: z.object({
          certificationTiming: z.enum(["BEFORE_ISSUE", "AFTER_LEDGER_COMMIT_OUTBOX", "REQUIRES_EXPERT_REVIEW"]),
          legalDeliveryWhenUncertified: z.enum(["BLOCK", "ALLOW_WITH_STATUS", "REQUIRES_EXPERT_REVIEW"]),
          offlineFinalNumberPolicy: z.enum(["PROHIBITED", "SERVER_ASSIGNED_ONLY", "REQUIRES_EXPERT_REVIEW"]),
  ...
  manualPortalFallback: z
    .array(
      regulatoryEnvelopeSchema.extend({
        value: z.object({
          availability: z.enum(["ALLOWED", "NOT_ALLOWED", "REQUIRES_EXPERT_REVIEW"]),
    ...
    .array(
      regulatoryEnvelopeSchema.extend({
        value: z.object({
          ...
          expectedArtifacts: z.array(eInvoicingArtifactExpectationSchema).min(1),
...
export type CountryPackEInvoicingMetadata = z.infer<typeof eInvoicingMetadataSchema>
