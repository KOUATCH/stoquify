...
function parseDate(value: Date | string | undefined, fallback = new Date()) {
  ...
  if (Number.isNaN(parsed.getTime())) throw new BusinessRuleError("Invalid payroll date.")
  return parsed
}

function decimal2(value: Prisma.Decimal.Value | bigint | null | undefined) {
  return new Prisma.Decimal(typeof value === "bigint" ? value.toString() : value ?? 0).toDecimalPlaces(2)
}
...
async function resolvePayrollCountryPackStatus(
  ...
  const countryCode = normalizeCountryCode(input.countryCode) ?? normalizeCountryCode(organization?.country)
  if (!countryCode) {
    throw new BusinessRuleError("Organization country is required before payroll country-pack calculation.")
  }
  ...
  const pensionRates = resolveRegulatoryParameter<CnpsPensionRates>("payroll.cnps.pensionRatesBps", {
    ...context,
    purpose: "PAYROLL_CNPS_PENSION",
  })
  const employerRules = resolveRegulatoryParameter<Record<string, unknown>>("payroll.cnps.employerRules", {
    ...context,
...
function amountForSource(source: PostingRuleAmountSource, amounts: PayrollPostingAmounts) {
  switch (source) {
...
async function createPayrollLedgerPosting(
  ...
  if (!rule) {
    throw new BusinessRuleError("Payroll posting requires configured SYSCOHADA payroll posting rules.")
  }
  ...
  if (!journal) {
    throw new BusinessRuleError("Payroll posting requires an active default payroll journal.")
  }
  ...
  const postingLines = rule.lines
    ...
    .map((line) => {
      ...
      if (!account) {
        throw new BusinessRuleError(`Payroll posting rule ${rule.code} line ${line.lineNumber} does not resolve to an account.`)
      }
      if (!account.isActive || account.deletedAt) {
        throw new BusinessRuleError(`Payroll posting rule ${rule.code} account ${account.code} is not active.`)
      }
      const childCount =
        "_count" in account ? Number((account as { _count?: { children?: number } })._count?.children ?? 0) : 0
      if (childCount > 0) {
        throw new BusinessRuleError(`Payroll posting rule ${rule.code} account ${account.code} must be a leaf account.`)
      }
      ...
      const amount = amountForSource(line.amountSource, input.amounts)
        ...
        .toDecimalPlaces(2)
      if (amount.lt(0)) {
        throw new BusinessRuleError(`Payroll posting rule ${rule.code} line ${line.lineNumber} produced a negative amount.`)
      }
  ...
  const journalEntry = await tx.journalEntry.create({
    data: {
      ...
      entryNumber: await nextPayrollEntryNumber(tx, input.organizationId, input.entryDate),
...
async function createPayrollPaymentLedgerPosting(
  ...
  if (!rule) {
    const ledgerBatch = await createPayrollLedgerBlocker(tx, {
      ...
      message: "Payroll payment release requires configured SYSCOHADA payroll payment posting rules.",
  ...
  const journalType = input.method === PaymentMethod.CASH ? JournalType.CASH : JournalType.BANK
  const journal = await tx.journal.findFirst({
    where: {
      organizationId: input.organizationId,
      type: journalType,
  ...
  const postingLines = rule.lines
    .filter((line) => conditionMatches(line.condition, { paymentMethod: input.method }))
    .map((line) => {
      const account = line.account ?? accountByMapping.get(normalizeMappingKey(line.mappingKey))
      if (!account) {
        throw new BusinessRuleError(`Payroll payment rule ${rule.code} line ${line.lineNumber} does not resolve to an account.`)
      }
      if (!account.isActive || account.deletedAt) {
        throw new BusinessRuleError(`Payroll payment rule ${rule.code} account ${account.code} is not active.`)
      }
      const childCount =
        "_count" in account ? Number((account as { _count?: { children?: number } })._count?.children ?? 0) : 0
      if (childCount > 0) {
        throw new BusinessRuleError(`Payroll payment rule ${rule.code} account ${account.code} must be a leaf account.`)
      }
      ...
      const amount = amountForSource(line.amountSource, amounts)
        ...
        .toDecimalPlaces(2)
      if (amount.lt(0)) {
        throw new BusinessRuleError(`Payroll payment rule ${rule.code} line ${line.lineNumber} produced a negative amount.`)
      }
      if (amount.eq(0)) return null
  ...
  if (postingLines.length === 0 || !debitTotal.eq(creditTotal)) {
    ...
    const ledgerBatch = await createPayrollLedgerBlocker(tx, {
      ...
      message,
      metadata: safeJson({ ...input.metadata, method: input.method, ruleCode: rule.code }),
    })
  ...
  const journalEntry = await tx.journalEntry.create({
    data: {
      ...
      periodId: input.periodId,
      postingBatchId: postedBatch.id,
      entryNumber: await nextPayrollEntryNumber(tx, input.organizationId, input.paymentDate),
...
export async function createPayrollPeriod(input: CreatePayrollPeriodInput, client: DbClient = db) {
  ...
  if (periodEnd < periodStart) {
    throw new BusinessRuleError("Payroll period end date must be on or after the start date.")
  }
...
export async function freezeAttendanceSnapshot(input: FreezeAttendanceSnapshotInput, client: DbClient = db) {
  ...
  return inTransaction(client, async (tx) => {
    const period = await tx.payrollPeriod.findFirst({
      where: { id: parsed.payrollPeriodId, organizationId: parsed.organizationId },
    })
    if (!period) throw new NotFoundError("Payroll period not found")
    if (period.status === PayrollPeriodStatus.CLOSED || period.status === PayrollPeriodStatus.POSTED) {
      throw new BusinessRuleError("Payroll attendance cannot be frozen for a closed or posted payroll period.")
    }
    ...
    if (!employee) throw new BusinessRuleError("Attendance can only be frozen for an active payroll employee.")
    ...
    if (existing) {
      if (existing.sourceHash !== sourceHash) {
        throw new ConflictError("Attendance snapshot replay changed the source payload.")
      }
      return { attendanceSnapshot: existing, created: false, businessEventId: null }
    }
    ...
    const eventResult = await recordBusinessEventInTx(tx as unknown as BusinessEventTx, {
      ...
      schemaVersion: 1,
      idempotencyKey: parsed.idempotencyKey ?? `attendance-freeze:${attendanceSnapshot.id}`,
      payload: {
        payrollPeriodId: period.id,
      ...
      metadata: { gate: "012-payroll-presence-engine", payrollPeriodId: period.id },
      outboxMessages: [
        {
    ...
    return { attendanceSnapshot, created: true, businessEventId: eventResult.event.id }
  })
}

export async function calculatePayrollRun(input: CalculatePayrollRunInput, client: DbClient = db) {
  ...
  return inTransaction(client, async (tx) => {
    ...
    if (existing) {
      ...
      return { payrollRun: existing, created: false, businessEventId: existing.postedBusinessEventId }
    }
    ...
    if (
      ...
      throw new BusinessRuleError("Payroll run can only be calculated for an open or input-locked period.")
    }
    ...
    if (employees.length === 0) {
      throw new BusinessRuleError("Payroll calculation requires at least one active employee.")
    }

    const pensionRates = countryPack.pensionRates.value ?? {}
    const employeeRateBps = Number(pensionRates.employee ?? 0)
    const employerRateBps = Number(pensionRates.employer ?? 0)
    ...
    const runLines = employees.map((employee) => {
      ...
      if (!contract) throw new BusinessRuleError(`Employee ${employee.displayName} has no active payroll contract.`)
      if (!attendance) throw new BusinessRuleError(`Employee ${employee.displayName} has no frozen attendance snapshot.`)

      const baseSalary = decimal2(contract.baseSalary)
      if (baseSalary.lte(0)) throw new BusinessRuleError(`Employee ${employee.displayName} has an invalid base salary.`)

      const scheduledMinutes = Math.max(0, Number(attendance.scheduledMinutes ?? 0))
      const paidMinutes = Math.max(0, Number(attendance.workedMinutes ?? 0) + Number(attendance.leaveMinutes ?? 0))
      ...
      const socialBase = monthlyCeiling && lineGross.gt(monthlyCeiling) ? monthlyCeiling : lineGross
      const employeeDeduction = socialBase.times(employeeRateBps).div(10000).toDecimalPlaces(2)
      const employerCharge = socialBase.times(employerRateBps).div(10000).toDecimalPlaces(2)
      ...
      employeeDeductionAmount = employeeDeductionAmount.plus(employeeDeduction).toDecimalPlaces(2)
      employerChargeAmount = employerChargeAmount.plus(employerCharge).toDecimalPlaces(2)

      const anomalyFlags = [
        ...(Number(attendance.overtimeMinutes ?? 0) > 0 ? ["OVERTIME_POLICY_REVIEW_REQUIRED"] : []),
        ...(employee.paymentDestinationHash ? [] : ["PAYMENT_DESTINATION_MISSING"]),
        ...(asRecord(countryPack.employerRules.value).payrollBaseRequiresCnpsReview
    ...
    const netPayableAmount = grossAmount.minus(employeeDeductionAmount).toDecimalPlaces(2)
    const runNumber = await nextPayrollRunNumber(tx, parsed.organizationId, runDate)
    const attendanceSnapshotHash = prefixedHash(runLines.map((line) => line.attendanceSnapshotId).sort())
    const calculationHash = prefixedHash(runLines.map((line) => line.calculationSnapshot))
    const ruleSetHash = prefixedHash(runLines.map((line) => line.ruleProvenance))
...
export async function approveAndPostPayrollRun(input: ApproveAndPostPayrollRunInput, client: DbClient = db) {
  ...
  return inTransaction(client, async (tx) => {
    ...
    if (run.status !== PayrollRunStatus.CALCULATED && run.status !== PayrollRunStatus.REVIEWED) {
      throw new BusinessRuleError("Only calculated or reviewed payroll runs can be approved and posted.")
    }
    if (run.lines.length === 0) throw new BusinessRuleError("Payroll run has no calculated employee lines.")
    ...
    const controlDecision = evaluateSensitiveAction({
      ...
      metadata: { runNumber: run.runNumber, payrollPeriodId: run.payrollPeriodId },
    })
    await auditSensitiveActionDecision(tx, controlDecision)
    assertSensitiveActionAllowed(controlDecision)

    const accountingPeriod = await getOpenPeriodForDate(parsed.organizationId, run.payrollPeriod.payDate, tx)
    const documentHash = parsed.documentHash ?? run.documentHash ?? prefixedHash({ payrollRunId: run.id, calculationHash: run.calculationHash })
    ...
    for (const [index, line] of run.lines.entries()) {
      ...
      const payslipHash = prefixedHash({
        ...
        employeeDeductionAmount: decimal2(line.employeeDeductionAmount).toFixed(2),
    ...
    const eventResult = await recordBusinessEventInTx(tx as unknown as BusinessEventTx, {
      ...
      payload: {
        ...
        employeeDeductionAmount: decimal2(run.employeeDeductionAmount).toFixed(2),
...
export async function releasePayrollPaymentBatch(input: ReleasePayrollPaymentBatchInput, client: DbClient = db) {
  ...
  if (!supportedMethods.has(method)) {
    throw new BusinessRuleError("Payroll payment method is not configured for salary disbursement posting.")
  }
  ...
  return inTransaction(client, async (tx) => {
    ...
    if (existing) {
      ...
      return {
        ...
        ledgerStatus: metadataString(existing.metadata, "ledgerStatus") ?? "IDEMPOTENT_REPLAY",
    ...
    if (parsed.requestedById === parsed.approvedById || parsed.requestedById === releasedById) {
      throw new BusinessRuleError("A separate approver and releaser are required before releasing payroll payments.")
    }
    ...
    if (run.status !== PayrollRunStatus.POSTED) {
      throw new BusinessRuleError("Payroll payments can only be released for posted payroll runs.")
    }
    if (!run.ledgerPostingBatchId || !run.postedBusinessEventId) {
      throw new BusinessRuleError("Payroll payment release requires posted payroll ledger and business-event evidence.")
    }
    if (run.payslips.length === 0) {
      throw new BusinessRuleError("Payroll payment release requires emitted payslips.")
    }
    ...
    const payslipIds = parsed.allocations.map((allocation) => allocation.payslipId)
    if (payslipIds.length !== new Set(payslipIds).size) {
      throw new BusinessRuleError("Payroll payment cannot contain duplicate payslip allocations.")
    }

    const payslipById = new Map(run.payslips.map((payslip) => [payslip.id, payslip]))
    const existingAllocations = payslipIds.length
      ? await tx.payrollPaymentAllocation.findMany({
          where: {
            ...
            payrollPaymentBatch: {
              status: { notIn: [PayrollPaymentBatchStatus.CANCELLED, PayrollPaymentBatchStatus.FAILED] },
            },
          },
        })
      : []
    ...
    for (const allocation of existingAllocations) {
      paidByPayslip.set(
        ...
        (paidByPayslip.get(allocation.payslipId) ?? new Prisma.Decimal(0)).plus(allocation.amount).toDecimalPlaces(2),
      )
    }
    ...
    const allocationPlans = parsed.allocations.map((allocation) => {
      ...
      if (!payslip || payslip.employeeId !== allocation.employeeId) {
        throw new BusinessRuleError("Payroll payment allocation must reference an emitted payslip from the selected run.")
      }
      if (payslip.status !== PayrollPayslipStatus.EMITTED) {
        throw new BusinessRuleError("Payroll payments can only be released for emitted payslips.")
      }
      if (!payslip.employee.paymentDestinationHash) {
        throw new BusinessRuleError(`Employee ${payslip.employee.displayName} has no approved payment destination evidence.`)
      }
      const amount = decimal2(allocation.amount)
      if (amount.lte(0)) throw new BusinessRuleError("Payroll payment allocation amount must be greater than zero.")
      const outstanding = decimal2(payslip.netPayableAmount)
        .minus(paidByPayslip.get(payslip.id) ?? new Prisma.Decimal(0))
        .toDecimalPlaces(2)
      if (amount.gt(outstanding)) {
        throw new BusinessRuleError("Payroll payment allocation exceeds unpaid payslip net payable.")
      }
    ...
    if (!totalPayment.eq(decimal2(run.netPayableAmount))) {
      throw new BusinessRuleError("Payroll payment batch total must equal the posted payroll net payable amount.")
    }

    const period = await getOpenPeriodForDate(parsed.organizationId, paymentDate, tx)
    const batchNumber = await nextPayrollPaymentBatchNumber(tx, parsed.organizationId, paymentDate)
    const evidencePayload = {
      ...
      method,
      paymentDate: paymentDate.toISOString(),
      allocationIds: allocationPlans.map((allocation) => allocation.payslip.id).sort(),
    ...
    const evidenceHash = prefixedHash({
      ...evidencePayload,
      destinationHashes: allocationPlans.map((allocation) => allocation.payslip.employee.paymentDestinationHash).sort(),
    })
        ...
        allocations: {
          create: allocationPlans.map((allocation) => ({
            ...
            metadata: safeJson({
              ...
              paymentDestinationHash: allocation.payslip.employee.paymentDestinationHash,
            }),
          })),
        },
      },
    ...
    const reconciliationResult = await queueOutboundPayrollPaymentReconciliation(tx, {
      organizationId: parsed.organizationId,
      payrollPaymentBatchId: paymentBatch.id,
      batchNumber: paymentBatch.batchNumber,
      payrollRunId: run.id,
    ...
    const eventResult = await recordBusinessEventInTx(tx as unknown as BusinessEventTx, {
      ...
      payload: {
        ...
        amount: totalPayment.toFixed(2),
        method,
        allocationIds: paymentBatch.allocations.map((allocation) => allocation.id),
        ledgerPostingBatchId: postingResult.ledgerBatch.id,
...
export async function preparePayrollDeclarations(input: PreparePayrollDeclarationsInput, client: DbClient = db) {
  ...
  return inTransaction(client, async (tx) => {
    ...
    if (run.status !== PayrollRunStatus.POSTED && run.status !== PayrollRunStatus.PAID) {
      throw new BusinessRuleError("Payroll declarations can only be prepared for posted payroll runs.")
    }
    ...
    try {
      ...
      declarationResolutionError = error instanceof Error ? error.message : "Payroll declaration country-pack resolution failed."
    }

    if (parsed.declarationTypes?.length) {
      const requested = new Set(parsed.declarationTypes.map((value) => value.trim().toUpperCase()))
      declarationConfig = declarationConfig.filter((entry) => requested.has(entry.declarationType.toUpperCase()))
    }
    ...
    for (const declaration of declarationConfig) {
      ...
      const amount = decimal2(run.employeeDeductionAmount).plus(decimal2(run.employerChargeAmount)).toDecimalPlaces(2)
      const dueDate = declaration.dueDate ? parseDate(declaration.dueDate) : null
      const payload = {
        payrollRunId: run.id,
        runNumber: run.runNumber,
        payrollPeriodId: run.payrollPeriodId,
    ...
    const eventResult = await recordBusinessEventInTx(tx as unknown as BusinessEventTx, {
      ...
      eventSource: "INTERNAL",
      schemaVersion: 1,
      idempotencyKey: parsed.idempotencyKey ?? `payroll-declaration-prepared:${run.id}`,
      payload: {
        payrollRunId: run.id,
        runNumber: run.runNumber,
        declarationIds: preparedDeclarations.map((declaration) => declaration.id),
        declarationTypes: preparedDeclarations.map((declaration) => declaration.declarationType),
      ...
      sourceType: AccountingSourceType.PAYROLL_RUN,
      sourceId: run.id,
      documentHash: prefixedHash(preparedDeclarations.map((declaration) => declaration.payloadHash).sort()),
      metadata: {
        gate: "012-payroll-presence-completion-gate",
        declarationResolutionError,
      },
...
export async function getPayrollWorkbenchData(
  ...
  const [
    ...
    client.payrollPeriod.count({
      where: { organizationId: input.organizationId, status: { in: [PayrollPeriodStatus.OPEN, PayrollPeriodStatus.INPUTS_LOCKED] } },
    }),
    client.payrollRun.count({
      where: { organizationId: input.organizationId, deletedAt: null, status: { in: [PayrollRunStatus.CALCULATED, PayrollRunStatus.REVIEWED] } },
    }),
    client.payrollRun.count({
      where: { organizationId: input.organizationId, deletedAt: null, status: PayrollRunStatus.POSTED },
    }),
    client.payrollPaymentBatch.count({
      where: { organizationId: input.organizationId, status: PayrollPaymentBatchStatus.RELEASED },
    }),
    client.payrollDeclaration.count({
      where: { organizationId: input.organizationId, status: { in: [PayrollDeclarationStatus.PREPARED, PayrollDeclarationStatus.REJECTED, PayrollDeclarationStatus.PAYMENT_DUE] } },
    }),
    client.ledgerPostingBatch.count({
      where: {
        organizationId: input.organizationId,
        sourceType: { in: [AccountingSourceType.PAYROLL_RUN, AccountingSourceType.PAYROLL_PAYMENT] },
        status: LedgerPostingBatchStatus.FAILED,
      },
    }),
    ...
    client.payrollRun.findMany({
      ...
      include: {
        payrollPeriod: true,
        _count: { select: { payslips: true, paymentBatches: true, declarations: true } },
      },
  ...
  return {
    ...
    queues: {
      ...
      paymentBatches: paymentBatches.map((batch) => ({
        ...
        status: batch.status,
        amount: decimal2(batch.amount).toFixed(2),
        ...
        ledgerBlockerMessage: metadataString(batch.metadata, "ledgerBlockerMessage"),
      })),
      declarations: declarations.map((declaration) => ({
        ...
        amount: decimal2(declaration.amount).toFixed(2),
        ...
        expertReviewRequired: Boolean(asRecord(declaration.metadata).expertReviewRequired),
      })),
