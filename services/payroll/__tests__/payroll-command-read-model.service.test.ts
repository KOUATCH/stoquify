...
describe("payroll command read model service", () => {
  it("composes trusted command data with blockers, role scope, redaction, evidence, and audit", async () => {
    ...
    expect(client.payrollAttendanceSnapshot.count).toHaveBeenCalledWith(expect.objectContaining({
  ...
  it("treats frozen but uncertified attendance as a readiness gap", async () => {
    ...
    expect(result.blockers).toEqual(expect.arrayContaining([
      expect.objectContaining({
        code: "PAYROLL_ATTENDANCE_FREEZE_GAP",
        domain: "attendance",
      }),
    ]))
    expect(result.readiness.attendance).toEqual(expect.objectContaining({
      state: "ACTION_REQUIRED",
      blockerCodes: expect.arrayContaining(["PAYROLL_ATTENDANCE_FREEZE_GAP"]),
    }))
  })
