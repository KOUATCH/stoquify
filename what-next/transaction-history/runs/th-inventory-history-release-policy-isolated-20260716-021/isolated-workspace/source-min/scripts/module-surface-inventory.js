...
function extractPermission(source) {
  ...
  const requireAllArray = source.match(/requireAllPermissions\(\s*\[([\s\S]*?)\]\s*,/)
  if (requireAllArray) return extractStringList(requireAllArray[1])
...
function detectGuard(source) {
  ...
  if (source.includes("requireAllPermissions(")) return "requireAllPermissions"
