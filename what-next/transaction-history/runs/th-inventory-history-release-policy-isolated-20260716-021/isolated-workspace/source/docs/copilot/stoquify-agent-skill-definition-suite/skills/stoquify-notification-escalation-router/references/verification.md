...
# Verification — Notification and Escalation Router

## Structural

- Validate frontmatter name equals `stoquify-notification-escalation-router` and contains only `name` and `description`.
- Validate `agents/openai.yaml` and confirm the default prompt invokes `$stoquify-notification-escalation-router`.
