...
# Verification — Redaction and Disclosure Policy

## Structural

- Validate frontmatter name equals `stoquify-redaction-disclosure-policy` and contains only `name` and `description`.
- Validate `agents/openai.yaml` and confirm the default prompt invokes `$stoquify-redaction-disclosure-policy`.
