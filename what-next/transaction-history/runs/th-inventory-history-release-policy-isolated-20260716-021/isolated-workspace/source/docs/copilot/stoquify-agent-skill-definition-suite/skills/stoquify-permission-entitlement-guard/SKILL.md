---
name: stoquify-permission-entitlement-guard
description: Use when Stoquify must evaluate tenant scope, RBAC, organization and location scope, module entitlement, action risk, segregation of duties, and fresh-auth requirements. Use for now-phase Copilot workflows requiring critical-risk controls, evidence, safe failure, and bounded autonomy.
---
...
Evaluate tenant scope, RBAC, organization and location scope, module entitlement, action risk, segregation of duties, and fresh-auth requirements.

## Scope and non-scope

- Operate as capability `S02` in the Stoquify Copilot registry.
...
4. Read only the relevant source services from: `services/auth`, `services/roles`, `services/modules`, `services/security`.
...
5. Perform only this skill's bounded responsibility: Evaluate tenant scope, RBAC, organization and location scope, module entitlement, action risk, segregation of duties, and fresh-auth requirements.
...
- Permitted service domains: `auth`, `roles`, `modules`, `security`.
...
Return DENIED with a policy-safe reason and approved workflow link; never propose a workaround.
