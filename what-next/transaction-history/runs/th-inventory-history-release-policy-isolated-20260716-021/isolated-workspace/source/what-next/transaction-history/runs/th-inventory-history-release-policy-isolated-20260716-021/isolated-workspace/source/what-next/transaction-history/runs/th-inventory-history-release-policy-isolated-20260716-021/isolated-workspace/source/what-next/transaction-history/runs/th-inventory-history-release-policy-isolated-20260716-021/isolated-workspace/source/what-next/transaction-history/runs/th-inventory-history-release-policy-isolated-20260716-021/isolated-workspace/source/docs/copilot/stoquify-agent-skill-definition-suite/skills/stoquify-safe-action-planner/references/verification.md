...
# Verification — Safe Action Planner

## Structural

- Validate frontmatter name equals `stoquify-safe-action-planner` and contains only `name` and `description`.
- Validate `agents/openai.yaml` and confirm the default prompt invokes `$stoquify-safe-action-planner`.
