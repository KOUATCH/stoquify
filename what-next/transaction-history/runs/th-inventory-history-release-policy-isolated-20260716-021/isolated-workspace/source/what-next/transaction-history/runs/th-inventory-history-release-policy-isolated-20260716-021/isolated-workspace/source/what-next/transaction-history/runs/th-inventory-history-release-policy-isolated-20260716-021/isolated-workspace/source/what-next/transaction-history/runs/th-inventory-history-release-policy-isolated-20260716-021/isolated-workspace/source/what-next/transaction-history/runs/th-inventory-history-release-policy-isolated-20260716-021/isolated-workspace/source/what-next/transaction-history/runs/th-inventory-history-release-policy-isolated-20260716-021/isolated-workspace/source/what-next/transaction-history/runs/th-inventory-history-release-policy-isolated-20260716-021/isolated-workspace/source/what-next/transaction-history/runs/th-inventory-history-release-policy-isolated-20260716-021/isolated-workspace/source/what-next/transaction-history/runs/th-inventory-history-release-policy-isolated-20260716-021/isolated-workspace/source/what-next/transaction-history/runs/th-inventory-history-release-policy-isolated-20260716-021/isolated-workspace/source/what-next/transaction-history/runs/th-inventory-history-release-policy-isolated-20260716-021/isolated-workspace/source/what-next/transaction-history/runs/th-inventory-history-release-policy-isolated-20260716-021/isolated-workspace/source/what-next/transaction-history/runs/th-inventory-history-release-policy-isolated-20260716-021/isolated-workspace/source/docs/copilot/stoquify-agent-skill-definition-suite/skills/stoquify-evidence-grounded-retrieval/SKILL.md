---
name: stoquify-evidence-grounded-retrieval
description: Use when Stoquify must retrieve only approved service or read-model evidence with source identity, scope, observed time, freshness, evidence grade, and redaction state. Use for now-phase Copilot workflows requiring high-risk controls, evidence, safe failure, and bounded autonomy.
---
...
Retrieve only approved service or read-model evidence with source identity, scope, observed time, freshness, evidence grade, and redaction state.

## Scope and non-scope

- Operate as capability `S03` in the Stoquify Copilot registry.
...
4. Read only the relevant source services from: `services/evidence`, `services/snapshots`, `services/bi`, `services/assurance`.
5. Read upstream capability evidence for: stoquify-trusted-context-resolver, stoquify-permission-entitlement-guard.
...
5. Perform only this skill's bounded responsibility: Retrieve only approved service or read-model evidence with source identity, scope, observed time, freshness, evidence grade, and redaction state.
...
- Permitted service domains: `evidence`, `snapshots`, `bi`, `assurance`.
...
Return UNAVAILABLE rather than infer unsupported business truth.
