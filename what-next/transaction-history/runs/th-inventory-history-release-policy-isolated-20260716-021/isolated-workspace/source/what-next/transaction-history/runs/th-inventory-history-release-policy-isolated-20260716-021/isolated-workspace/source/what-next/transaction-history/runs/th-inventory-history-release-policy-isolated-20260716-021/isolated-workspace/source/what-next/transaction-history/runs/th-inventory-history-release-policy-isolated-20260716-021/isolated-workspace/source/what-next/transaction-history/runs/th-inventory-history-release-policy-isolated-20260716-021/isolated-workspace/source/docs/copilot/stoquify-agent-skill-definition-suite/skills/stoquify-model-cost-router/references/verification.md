...
# Verification — Model and Cost Router

## Structural

- Validate frontmatter name equals `stoquify-model-cost-router` and contains only `name` and `description`.
- Validate `agents/openai.yaml` and confirm the default prompt invokes `$stoquify-model-cost-router`.
