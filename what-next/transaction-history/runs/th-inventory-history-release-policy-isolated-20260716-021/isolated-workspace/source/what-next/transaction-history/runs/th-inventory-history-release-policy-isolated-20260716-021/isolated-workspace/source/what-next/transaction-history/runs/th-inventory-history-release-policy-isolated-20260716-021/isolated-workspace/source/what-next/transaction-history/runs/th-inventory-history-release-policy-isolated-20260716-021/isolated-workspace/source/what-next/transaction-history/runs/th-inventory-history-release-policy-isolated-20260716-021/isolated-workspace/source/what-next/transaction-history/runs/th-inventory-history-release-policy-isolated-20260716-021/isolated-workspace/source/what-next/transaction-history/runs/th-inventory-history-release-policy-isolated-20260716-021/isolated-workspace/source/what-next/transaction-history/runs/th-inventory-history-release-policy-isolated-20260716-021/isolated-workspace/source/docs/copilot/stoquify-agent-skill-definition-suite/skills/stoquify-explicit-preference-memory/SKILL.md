---
name: stoquify-explicit-preference-memory
description: Use when Stoquify must store only user-approved preferences and saved routines with purpose, scope, expiry, correction, export, and deletion controls. Use for next-phase Copilot workflows requiring critical-risk controls, evidence, safe failure, and bounded autonomy.
---
...
Store only user-approved preferences and saved routines with purpose, scope, expiry, correction, export, and deletion controls.

## Scope and non-scope

- Operate as capability `S14` in the Stoquify Copilot registry.
...
4. Read only the relevant source services from: `services/users`, `services/organization`, `services/security`.
5. Read upstream capability evidence for: stoquify-trusted-context-resolver, stoquify-permission-entitlement-guard, stoquify-redaction-disclosure-policy.
...
5. Perform only this skill's bounded responsibility: Store only user-approved preferences and saved routines with purpose, scope, expiry, correction, export, and deletion controls.
...
- Permitted service domains: `users`, `organization`, `security`.
...
Return UNKNOWN; never infer authority or retain payroll, secrets, raw financial records, or unapproved personal data.
