---
name: stoquify-inventory-risk-replenishment
description: Use when Stoquify must estimate location-specific stockout or overstock risk and propose reorder or transfer using availability, commitments, velocity, lead time, confidence, and cash exposure. Use for now-phase Copilot workflows requiring high-risk controls, evidence, safe failure, and bounded autonomy.
---
...
Estimate location-specific stockout or overstock risk and propose reorder or transfer using availability, commitments, velocity, lead time, confidence, and cash exposure.

## Scope and non-scope

- Operate as capability `S21` in the Stoquify Copilot registry.
...
- Do not perform Tier 4 actions or exceed autonomy class `draft`.
...
4. Read only the relevant source services from: `services/inventory`, `services/item`, `services/location`, `services/supplier`, `services/purchase-order`, `services/analytics`.
5. Read upstream capability evidence for: stoquify-trusted-context-resolver, stoquify-permission-entitlement-guard, stoquify-evidence-grounded-retrieval, stoquify-freshness-trust-evaluator, stoquify-offline-replay-awareness.
...
5. Perform only this skill's bounded responsibility: Estimate location-specific stockout or overstock risk and propose reorder or transfer using availability, commitments, velocity, lead time, confidence, and cash exposure.
...
- Permitted service domains: `inventory`, `item`, `location`, `supplier`, `purchase-order`, `analytics`.
...
Risk class: `high`. Autonomy ceiling: `draft`. Default classification is `confidential`; payroll, secrets, credentials, bank destinations, identity data, and sensitive financial evidence are `restricted`. Minimize and redact before model use.

## Safe failure and stop conditions

Return a recommendation gap; never approximate missing inventory states or approve a purchase order.
