...
# Verification — Inventory Risk and Replenishment

## Structural

- Validate frontmatter name equals `stoquify-inventory-risk-replenishment` and contains only `name` and `description`.
- Validate `agents/openai.yaml` and confirm the default prompt invokes `$stoquify-inventory-risk-replenishment`.
...
- Domain-specific risk cases for `high` risk and `draft` autonomy.
