---
name: stoquify-model-cost-router
description: Use when Stoquify must select an approved provider and model by sensitivity, complexity, latency, evaluation status, and tenant budget; enforce context and cost ceilings. Use for next-phase Copilot workflows requiring high-risk controls, evidence, safe failure, and bounded autonomy.
---
...
Select an approved provider and model by sensitivity, complexity, latency, evaluation status, and tenant budget; enforce context and cost ceilings.

## Scope and non-scope

- Operate as capability `S13` in the Stoquify Copilot registry.
...
4. Read only the relevant source services from: `services/analytics`, `services/controls`, `services/assurance`.
5. Read upstream capability evidence for: stoquify-trusted-context-resolver, stoquify-permission-entitlement-guard, stoquify-agent-evidence-recorder, stoquify-freshness-trust-evaluator.
...
5. Perform only this skill's bounded responsibility: Select an approved provider and model by sensitivity, complexity, latency, evaluation status, and tenant budget; enforce context and cost ceilings.
...
- Permitted service domains: `analytics`, `controls`, `assurance`.
...
Use an approved fallback or return UNAVAILABLE without bypassing data-region or sensitivity policy.
