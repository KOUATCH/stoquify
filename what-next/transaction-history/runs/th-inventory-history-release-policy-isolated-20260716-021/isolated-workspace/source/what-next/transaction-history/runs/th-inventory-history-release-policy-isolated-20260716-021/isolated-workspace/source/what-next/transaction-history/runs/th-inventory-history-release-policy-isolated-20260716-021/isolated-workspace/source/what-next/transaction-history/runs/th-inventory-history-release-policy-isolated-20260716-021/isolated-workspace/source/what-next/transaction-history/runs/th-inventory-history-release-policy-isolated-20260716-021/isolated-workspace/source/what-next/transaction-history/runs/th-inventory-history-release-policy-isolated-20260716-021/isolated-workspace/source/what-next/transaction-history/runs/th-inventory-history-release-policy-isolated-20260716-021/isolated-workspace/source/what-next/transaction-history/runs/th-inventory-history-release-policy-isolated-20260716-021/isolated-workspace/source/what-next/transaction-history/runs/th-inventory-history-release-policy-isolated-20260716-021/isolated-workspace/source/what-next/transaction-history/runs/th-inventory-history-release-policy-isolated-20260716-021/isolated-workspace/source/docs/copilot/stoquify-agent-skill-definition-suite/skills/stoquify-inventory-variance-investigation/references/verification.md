...
# Verification — Inventory Variance Investigation

## Structural

- Validate frontmatter name equals `stoquify-inventory-variance-investigation` and contains only `name` and `description`.
- Validate `agents/openai.yaml` and confirm the default prompt invokes `$stoquify-inventory-variance-investigation`.
...
- Domain-specific risk cases for `high` risk and `read` autonomy.
