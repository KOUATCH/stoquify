---
name: stoquify-cash-exception-triage
description: Use when Stoquify must classify and rank provider, statement, settlement, suspense, and reconciliation exceptions with evidence gaps and close impact. Use for now-phase Copilot workflows requiring critical-risk controls, evidence, safe failure, and bounded autonomy.
---
...
Classify and rank provider, statement, settlement, suspense, and reconciliation exceptions with evidence gaps and close impact.

## Scope and non-scope

- Operate as capability `S19` in the Stoquify Copilot registry.
...
4. Read only the relevant source services from: `services/payments`, `services/reconciliation`, `services/cash-command`, `services/accounting`.
5. Read upstream capability evidence for: stoquify-trusted-context-resolver, stoquify-permission-entitlement-guard, stoquify-evidence-grounded-retrieval, stoquify-redaction-disclosure-policy, stoquify-freshness-trust-evaluator, stoquify-exception-prioritizer, stoquify-cross-domain-root-cause-trace.
...
5. Perform only this skill's bounded responsibility: Classify and rank provider, statement, settlement, suspense, and reconciliation exceptions with evidence gaps and close impact.
...
- Permitted service domains: `payments`, `reconciliation`, `cash-command`, `accounting`.
...
Return a review queue; never resolve, post, certify, or move funds.
