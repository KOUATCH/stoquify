---
name: stoquify-trusted-context-resolver
description: Use when Stoquify must resolve server-derived tenant, actor, organization, role, location, route or record, locale, currency, period, and correlation context before any reasoning or tool selection. Use for now-phase Copilot workflows requiring critical-risk controls, evidence, safe failure, and bounded autonomy.
---
...
Resolve server-derived tenant, actor, organization, role, location, route or record, locale, currency, period, and correlation context before any reasoning or tool selection.

## Scope and non-scope

- Operate as capability `S01` in the Stoquify Copilot registry.
...
4. Read only the relevant source services from: `services/auth`, `services/organization`, `services/location`, `services/roles`, `services/modules`.
5. Read upstream capability evidence for: none.
...
5. Perform only this skill's bounded responsibility: Resolve server-derived tenant, actor, organization, role, location, route or record, locale, currency, period, and correlation context before any reasoning or tool selection.
...
- Permitted service domains: `auth`, `organization`, `location`, `roles`, `modules`.
...
Return BLOCKED when context is ambiguous, absent, contradictory, or not server-verifiable.
