---
name: stoquify-inventory-variance-investigation
description: Use when Stoquify must explain negative stock, count variance, transfer conflict, replay conflict, and projection drift from service-owned event history. Use for now-phase Copilot workflows requiring high-risk controls, evidence, safe failure, and bounded autonomy.
---
...
Explain negative stock, count variance, transfer conflict, replay conflict, and projection drift from service-owned event history.

## Scope and non-scope

- Operate as capability `S22` in the Stoquify Copilot registry.
...
4. Read only the relevant source services from: `services/inventory`, `services/events`, `services/history`, `services/pos`, `services/assurance`.
5. Read upstream capability evidence for: stoquify-trusted-context-resolver, stoquify-permission-entitlement-guard, stoquify-evidence-grounded-retrieval, stoquify-freshness-trust-evaluator, stoquify-offline-replay-awareness, stoquify-cross-domain-root-cause-trace.
...
5. Perform only this skill's bounded responsibility: Explain negative stock, count variance, transfer conflict, replay conflict, and projection drift from service-owned event history.
...
- Permitted service domains: `inventory`, `events`, `history`, `pos`, `assurance`.
...
Return PARTIAL when the event chain is incomplete; never post an adjustment or write-off.
