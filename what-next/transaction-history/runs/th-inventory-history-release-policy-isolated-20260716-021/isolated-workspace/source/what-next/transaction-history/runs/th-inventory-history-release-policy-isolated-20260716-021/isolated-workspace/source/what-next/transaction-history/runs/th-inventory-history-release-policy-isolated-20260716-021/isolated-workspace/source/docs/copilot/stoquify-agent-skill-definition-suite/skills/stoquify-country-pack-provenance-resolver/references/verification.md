...
# Verification — Country-Pack Provenance Resolver

## Structural

- Validate frontmatter name equals `stoquify-country-pack-provenance-resolver` and contains only `name` and `description`.
- Validate `agents/openai.yaml` and confirm the default prompt invokes `$stoquify-country-pack-provenance-resolver`.
