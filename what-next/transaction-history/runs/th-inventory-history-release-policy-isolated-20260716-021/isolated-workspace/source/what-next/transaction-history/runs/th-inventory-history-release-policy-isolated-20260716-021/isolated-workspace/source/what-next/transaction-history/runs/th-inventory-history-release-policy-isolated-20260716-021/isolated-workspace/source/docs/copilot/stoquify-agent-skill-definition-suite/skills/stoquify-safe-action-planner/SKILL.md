---
name: stoquify-safe-action-planner
description: Use when Stoquify must convert an authorized request into bounded steps, required deterministic tools, preconditions, evidence, approvals, expiry, idempotency, and rollback or compensation notes. Use for now-phase Copilot workflows requiring high-risk controls, evidence, safe failure, and bounded autonomy.
---
...
Convert an authorized request into bounded steps, required deterministic tools, preconditions, evidence, approvals, expiry, idempotency, and rollback or compensation notes.

## Scope and non-scope

- Operate as capability `S04` in the Stoquify Copilot registry.
...
4. Read only the relevant source services from: `services/controls`, `services/assurance`, `services/events`.
5. Read upstream capability evidence for: stoquify-trusted-context-resolver, stoquify-permission-entitlement-guard, stoquify-evidence-grounded-retrieval.
...
5. Perform only this skill's bounded responsibility: Convert an authorized request into bounded steps, required deterministic tools, preconditions, evidence, approvals, expiry, idempotency, and rollback or compensation notes.
...
- Permitted service domains: `controls`, `assurance`, `events`.
...
Produce a proposal only; return BLOCKED when no safe plan exists.
