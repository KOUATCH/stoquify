...
# Verification — Cash Exception Triage

## Structural

- Validate frontmatter name equals `stoquify-cash-exception-triage` and contains only `name` and `description`.
- Validate `agents/openai.yaml` and confirm the default prompt invokes `$stoquify-cash-exception-triage`.
