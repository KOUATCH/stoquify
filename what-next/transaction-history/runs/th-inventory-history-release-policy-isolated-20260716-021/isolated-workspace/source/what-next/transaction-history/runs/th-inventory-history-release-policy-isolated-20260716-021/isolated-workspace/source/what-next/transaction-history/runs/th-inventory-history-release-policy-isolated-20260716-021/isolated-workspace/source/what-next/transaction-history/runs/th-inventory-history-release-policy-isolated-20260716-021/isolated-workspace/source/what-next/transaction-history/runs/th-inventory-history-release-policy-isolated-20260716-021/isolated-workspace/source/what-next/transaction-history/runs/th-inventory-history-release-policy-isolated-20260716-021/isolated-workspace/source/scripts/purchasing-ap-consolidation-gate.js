...
function buildPurchasingAPReadiness(root = process.cwd(), options = {}) {
  ...
  const receiptSection = section(purchaseOrder, "export async function receiveItems", "export async function bulkUpdateStatus")
  const apPostingSection = section(apControl, "async function createAPLedgerPosting", "async function queueOutboundSupplierPaymentReconciliation")

  const checks = [
    {
      id: "purchase_order_maker_checker",
      ready: purchaseOrder.includes("po.createdById === approvedById") &&
    ...
    },
    {
      ...
      ready: purchaseOrder.includes("postGoodsReceiptStock(") && markersInOrder(receiptSection, [
    ...
    },
    {
      ...
      ready: purchaseOrder.includes("Cannot replace purchase order lines after receipt or invoice evidence exists.") &&
    ...
    },
    {
      ...
      ready: apControl.includes("tx.goodsReceiptLine.findFirst(") &&
    ...
    },
    {
      ...
      ready: apControl.includes("tx.threeWayMatch.create(") &&
    ...
    },
    {
      ...
      ready: apPostingSection.includes("createLedgerPostingBatch(") &&
    ...
    },
    {
      ...
      ready: markersInOrder(apPostingSection, [
    ...
    },
    {
      ...
      ready: apControl.includes("approveSupplierPaymentWithControls") &&
    ...
    },
    {
      ...
      ready: apControl.includes("SupplierBankAccountStatus.APPROVED") &&
    ...
    },
    {
      ...
      ready: assurance.includes('"purchasing_ap.po_approval_receipt_trace.required"') &&
        ...
        packageJson.includes('"purchasing:ap:gate"') && packageJson.includes("npm run purchasing:ap:gate"),
    },
  ]
