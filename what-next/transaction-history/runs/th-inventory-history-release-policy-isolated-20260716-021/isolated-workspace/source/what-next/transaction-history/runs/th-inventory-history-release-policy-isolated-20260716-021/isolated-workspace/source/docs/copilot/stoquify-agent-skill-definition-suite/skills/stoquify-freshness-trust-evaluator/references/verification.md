...
# Verification — Freshness and Trust Evaluator

## Structural

- Validate frontmatter name equals `stoquify-freshness-trust-evaluator` and contains only `name` and `description`.
- Validate `agents/openai.yaml` and confirm the default prompt invokes `$stoquify-freshness-trust-evaluator`.
