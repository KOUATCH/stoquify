...
# Verification — Agent Evidence Recorder

## Structural

- Validate frontmatter name equals `stoquify-agent-evidence-recorder` and contains only `name` and `description`.
- Validate `agents/openai.yaml` and confirm the default prompt invokes `$stoquify-agent-evidence-recorder`.
