...
# Verification — Permission and Entitlement Guard

## Structural

- Validate frontmatter name equals `stoquify-permission-entitlement-guard` and contains only `name` and `description`.
- Validate `agents/openai.yaml` and confirm the default prompt invokes `$stoquify-permission-entitlement-guard`.
