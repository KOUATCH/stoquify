...
# Verification — Idempotent Tool Executor

## Structural

- Validate frontmatter name equals `stoquify-idempotent-tool-executor` and contains only `name` and `description`.
- Validate `agents/openai.yaml` and confirm the default prompt invokes `$stoquify-idempotent-tool-executor`.
...
- Domain-specific risk cases for `critical` risk and `controlled-write` autonomy.
