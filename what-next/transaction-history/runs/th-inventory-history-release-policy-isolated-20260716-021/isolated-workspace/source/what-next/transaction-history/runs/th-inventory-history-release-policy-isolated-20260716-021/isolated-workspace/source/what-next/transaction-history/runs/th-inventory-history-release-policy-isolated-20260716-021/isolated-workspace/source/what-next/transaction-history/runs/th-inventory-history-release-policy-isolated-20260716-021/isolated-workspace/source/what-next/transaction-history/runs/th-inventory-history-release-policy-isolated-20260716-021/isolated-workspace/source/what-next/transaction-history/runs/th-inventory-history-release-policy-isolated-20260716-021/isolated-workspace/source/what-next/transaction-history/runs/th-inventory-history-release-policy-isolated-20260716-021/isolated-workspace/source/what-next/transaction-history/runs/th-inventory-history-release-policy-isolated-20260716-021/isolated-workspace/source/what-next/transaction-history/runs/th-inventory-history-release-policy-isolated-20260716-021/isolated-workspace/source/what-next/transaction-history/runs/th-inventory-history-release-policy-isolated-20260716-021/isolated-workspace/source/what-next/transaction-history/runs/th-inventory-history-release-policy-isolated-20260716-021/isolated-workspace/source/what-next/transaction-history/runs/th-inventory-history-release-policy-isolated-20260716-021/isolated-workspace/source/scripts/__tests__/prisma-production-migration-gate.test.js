...
function makeRepo(sql = 'CREATE TABLE "example" ("id" TEXT PRIMARY KEY);\n') {
  ...
  fs.writeFileSync(path.join(root, "prisma/migration-risk-approvals.json"), JSON.stringify({ version: 1, approvals: [] }), "utf8")
  fs.writeFileSync(path.join(root, "package.json"), JSON.stringify({
    scripts: {
      build: "npm run release:secrets:preflight && npm run prisma:migrate:deploy:safe && npm run build:linted",
      "policy:gates": "npm run prisma:migration:safety:gate && npm run release:evidence:gate",
      "verify:release": "npm run verify:repo && npm run prisma:migration:release:preflight",
    },
  }), "utf8")
...
describe("Prisma production migration gate", () => {
  it("passes static checks and skips local database mutation", () => {
    const { root } = makeRepo()
    const report = buildPrismaMigrationReadiness(root, { environment: "local", environmentValues: {} })

    expect(report.summary).toMatchObject({ status: "ready", readyCount: 8, checkCount: 8, blockerCount: 0 })
    expect(report.deployment).toMatchObject({ shouldDeploy: false, reason: "non_production_default_skip" })
  ...
  it("skips preview migration by default even when DATABASE_URL exists", () => {
    ...
    const report = buildPrismaMigrationReadiness(root, {
      environment: "preview",
      environmentValues: { DATABASE_URL: "postgresql://user:preview-secret@db.example.com:5432/preview" },
    })
  ...
  it("blocks production when DATABASE_URL is missing or local", () => {
    const { root } = makeRepo()
    const missing = buildPrismaMigrationReadiness(root, { environment: "production", environmentValues: {} })
    const local = buildPrismaMigrationReadiness(root, {
      environment: "production",
      environmentValues: { DATABASE_URL: "postgresql://user:password@localhost:5432/stoquify" },
    })
  ...
  it("executes Prisma deploy for a safe production target", () => {
    const { root } = makeRepo()
    const environmentValues = { DATABASE_URL: "postgresql://user:production-secret@db.example.com:5432/stoquify" }
    const report = buildPrismaMigrationReadiness(root, { environment: "production", environmentValues })
    const calls = []
    ...
    expect(report.execution).toMatchObject({ attempted: true, status: "succeeded", exitCode: 0 })
  ...
  it("accepts a reviewed destructive operation only for the exact file hash", () => {
    ...
    const approved = buildPrismaMigrationReadiness(root, { environment: "local", environmentValues: {} })
    expect(approved.summary.status).toBe("ready")
    expect(approved.findings[0].approved).toBe(true)

    fs.appendFileSync(path.join(root, migration), "-- changed after approval\n", "utf8")
    const changed = buildPrismaMigrationReadiness(root, { environment: "local", environmentValues: {} })
  ...
  it("requires explicit isolated-target attestation for forced preview deploys", () => {
    ...
    const report = buildPrismaMigrationReadiness(root, { environment: "preview", environmentValues })

    expect(report.blockers).toContain("preview_database_must_be_attested_isolated")
  })
  ...
  it("never writes database credentials into evidence", () => {
    const { root } = makeRepo()
    const databaseUrl = "postgresql://user:do-not-print-this@db.example.com:5432/stoquify"
    const report = buildPrismaMigrationReadiness(root, {
      environment: "production",
