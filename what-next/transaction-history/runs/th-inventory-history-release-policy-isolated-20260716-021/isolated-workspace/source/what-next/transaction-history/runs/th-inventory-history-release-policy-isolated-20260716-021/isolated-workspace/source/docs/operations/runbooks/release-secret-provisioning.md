...
This runbook owns the production configuration boundary for Stoquify's public identity abuse hashing and public receipt signing secrets. Secret values belong in the Vercel project environment, never in source control, saved evidence, CI output, issue trackers, or chat transcripts.
...
Both values must contain at least 12 distinct characters. They must differ from each other and from `AUTH_SECRET` or `NEXTAUTH_SECRET`.
...
3. Add both variables to the **Production** environment. Do not expose them to Preview or Development unless that environment genuinely exercises public links.
...
5. Confirm the build log reports six ready checks, zero blockers, and `Secret value printed: no`.
