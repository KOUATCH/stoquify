...
# Verification — Evidence-Grounded Retrieval

## Structural

- Validate frontmatter name equals `stoquify-evidence-grounded-retrieval` and contains only `name` and `description`.
- Validate `agents/openai.yaml` and confirm the default prompt invokes `$stoquify-evidence-grounded-retrieval`.
