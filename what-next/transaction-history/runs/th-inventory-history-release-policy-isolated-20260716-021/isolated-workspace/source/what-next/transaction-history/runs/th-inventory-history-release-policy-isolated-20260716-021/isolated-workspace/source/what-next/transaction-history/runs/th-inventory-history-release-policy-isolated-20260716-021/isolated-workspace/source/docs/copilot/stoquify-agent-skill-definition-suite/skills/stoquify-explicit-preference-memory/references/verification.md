...
# Verification — Explicit Preference Memory

## Structural

- Validate frontmatter name equals `stoquify-explicit-preference-memory` and contains only `name` and `description`.
- Validate `agents/openai.yaml` and confirm the default prompt invokes `$stoquify-explicit-preference-memory`.
