...
# Verification — Trusted Context Resolver

## Structural

- Validate frontmatter name equals `stoquify-trusted-context-resolver` and contains only `name` and `description`.
- Validate `agents/openai.yaml` and confirm the default prompt invokes `$stoquify-trusted-context-resolver`.
