---
name: aqstoqflow-module-billing-provisioning-boundary
description: Design AqStoqFlow provider-independent billing and provisioning boundaries for modules. Use when defining billing adapters, idempotent provisioning, dunning, suspension, reactivation, manual overrides, reconciliation evidence, and safeguards so billing webhooks do not directly become runtime entitlement truth.
---
...
Billing-provider boundary and safe provisioning workflow
...
3. Execute only this lane's scope: Define provider adapter, idempotency keys, subscription reconciliation, override audit, dunning, suspension, read-only, and reactivation workflows.
