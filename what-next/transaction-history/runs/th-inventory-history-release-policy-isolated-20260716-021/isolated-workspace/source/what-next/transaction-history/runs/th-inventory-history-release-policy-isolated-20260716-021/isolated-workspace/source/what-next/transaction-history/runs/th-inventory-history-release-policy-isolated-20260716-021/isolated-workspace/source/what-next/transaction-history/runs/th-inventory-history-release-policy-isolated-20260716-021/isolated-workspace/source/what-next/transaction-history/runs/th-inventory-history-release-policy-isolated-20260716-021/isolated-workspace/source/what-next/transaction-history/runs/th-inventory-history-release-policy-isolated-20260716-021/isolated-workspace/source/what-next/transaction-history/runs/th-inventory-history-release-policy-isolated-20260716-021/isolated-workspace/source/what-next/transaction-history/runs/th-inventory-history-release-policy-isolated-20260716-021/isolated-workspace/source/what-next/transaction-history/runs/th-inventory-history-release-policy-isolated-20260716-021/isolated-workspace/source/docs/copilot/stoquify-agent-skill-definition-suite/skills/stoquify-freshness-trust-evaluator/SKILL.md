---
name: stoquify-freshness-trust-evaluator
description: Use when Stoquify must interpret snapshot age, evidence grade, blockers, certification state, provisional state, contradictions, and unsupported claims. Use for now-phase Copilot workflows requiring high-risk controls, evidence, safe failure, and bounded autonomy.
---
...
Interpret snapshot age, evidence grade, blockers, certification state, provisional state, contradictions, and unsupported claims.

## Scope and non-scope

- Operate as capability `S09` in the Stoquify Copilot registry.
...
4. Read only the relevant source services from: `services/snapshots`, `services/evidence`, `services/assurance`.
5. Read upstream capability evidence for: stoquify-evidence-grounded-retrieval, stoquify-agent-evidence-recorder.
...
5. Perform only this skill's bounded responsibility: Interpret snapshot age, evidence grade, blockers, certification state, provisional state, contradictions, and unsupported claims.
...
- Permitted service domains: `snapshots`, `evidence`, `assurance`.
...
Return top-level `partial` or `unavailable` with `result.reason_code = STALE` when freshness fails; never hide trust limitations.
