---
name: stoquify-po-receipt-invoice-variance
description: Use when Stoquify must explain purchase-order, receipt, and supplier-invoice differences and prepare an evidence-linked resolution packet. Use for next-phase Copilot workflows requiring critical-risk controls, evidence, safe failure, and bounded autonomy.
---

# PO Receipt Invoice Variance

## Purpose

Explain purchase-order, receipt, and supplier-invoice differences and prepare an evidence-linked resolution packet.

## Scope and non-scope

- Operate as capability `S23` in the Stoquify Copilot registry.
...
4. Read only the relevant source services from: `services/purchasing`, `services/purchase-order`, `services/supplier`, `services/accounting`, `services/evidence`.
5. Read upstream capability evidence for: stoquify-trusted-context-resolver, stoquify-permission-entitlement-guard, stoquify-evidence-grounded-retrieval, stoquify-safe-action-planner, stoquify-freshness-trust-evaluator, stoquify-cross-domain-root-cause-trace.
...
5. Perform only this skill's bounded responsibility: Explain purchase-order, receipt, and supplier-invoice differences and prepare an evidence-linked resolution packet.
...
- Permitted service domains: `purchasing`, `purchase-order`, `supplier`, `accounting`, `evidence`.
...
Return BLOCKED when any leg is missing; never approve its own resolution or release payment.
