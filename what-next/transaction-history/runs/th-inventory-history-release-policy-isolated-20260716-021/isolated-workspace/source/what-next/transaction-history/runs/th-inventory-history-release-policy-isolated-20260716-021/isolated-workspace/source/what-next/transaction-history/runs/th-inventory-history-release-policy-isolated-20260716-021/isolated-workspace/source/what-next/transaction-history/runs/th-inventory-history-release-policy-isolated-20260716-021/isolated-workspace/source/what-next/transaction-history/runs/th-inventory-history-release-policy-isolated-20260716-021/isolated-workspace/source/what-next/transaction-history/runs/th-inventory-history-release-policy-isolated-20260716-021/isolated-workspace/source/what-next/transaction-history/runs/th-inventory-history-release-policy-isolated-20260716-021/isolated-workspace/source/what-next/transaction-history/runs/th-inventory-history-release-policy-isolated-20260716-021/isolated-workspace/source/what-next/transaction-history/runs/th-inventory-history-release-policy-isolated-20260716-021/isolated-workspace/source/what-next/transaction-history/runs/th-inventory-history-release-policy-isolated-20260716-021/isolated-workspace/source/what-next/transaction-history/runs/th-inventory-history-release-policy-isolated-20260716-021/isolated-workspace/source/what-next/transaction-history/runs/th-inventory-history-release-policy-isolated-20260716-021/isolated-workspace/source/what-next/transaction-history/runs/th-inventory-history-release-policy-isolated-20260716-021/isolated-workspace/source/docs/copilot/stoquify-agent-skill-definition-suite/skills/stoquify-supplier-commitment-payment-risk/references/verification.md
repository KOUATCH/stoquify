...
# Verification — Supplier Commitment and Payment Risk

## Structural

- Validate frontmatter name equals `stoquify-supplier-commitment-payment-risk` and contains only `name` and `description`.
- Validate `agents/openai.yaml` and confirm the default prompt invokes `$stoquify-supplier-commitment-payment-risk`.
...
- Domain-specific risk cases for `critical` risk and `read` autonomy.
