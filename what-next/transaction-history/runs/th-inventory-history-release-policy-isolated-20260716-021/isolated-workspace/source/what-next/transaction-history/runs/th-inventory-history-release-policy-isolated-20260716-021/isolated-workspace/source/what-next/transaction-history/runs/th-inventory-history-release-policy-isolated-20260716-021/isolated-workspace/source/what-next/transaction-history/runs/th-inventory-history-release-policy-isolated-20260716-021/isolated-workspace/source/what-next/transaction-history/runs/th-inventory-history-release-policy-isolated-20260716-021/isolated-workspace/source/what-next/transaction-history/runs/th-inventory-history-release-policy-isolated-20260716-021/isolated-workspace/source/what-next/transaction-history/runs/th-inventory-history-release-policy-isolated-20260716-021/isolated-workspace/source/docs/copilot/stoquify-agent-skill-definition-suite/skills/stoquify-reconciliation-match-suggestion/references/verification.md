...
# Verification — Reconciliation Match Suggestion

## Structural

- Validate frontmatter name equals `stoquify-reconciliation-match-suggestion` and contains only `name` and `description`.
- Validate `agents/openai.yaml` and confirm the default prompt invokes `$stoquify-reconciliation-match-suggestion`.
