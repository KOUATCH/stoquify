...
# Verification — Compliance Readiness Explanation

## Structural

- Validate frontmatter name equals `stoquify-compliance-readiness-explanation` and contains only `name` and `description`.
- Validate `agents/openai.yaml` and confirm the default prompt invokes `$stoquify-compliance-readiness-explanation`.
