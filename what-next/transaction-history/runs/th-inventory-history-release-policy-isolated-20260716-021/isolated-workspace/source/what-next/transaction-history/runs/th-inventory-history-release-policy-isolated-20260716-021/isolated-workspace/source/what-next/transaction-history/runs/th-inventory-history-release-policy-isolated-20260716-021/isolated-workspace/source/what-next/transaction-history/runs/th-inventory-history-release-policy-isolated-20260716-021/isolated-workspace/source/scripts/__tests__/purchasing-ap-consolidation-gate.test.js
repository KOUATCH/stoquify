...
const {
  ...
  gateResultForReport,
} = require("../purchasing-ap-consolidation-gate")
...
function writeReadyFixture(root) {
  write(root, "services/purchase-order/purchase-order.service.ts", [
  ...
  write(root, "services/purchasing/ap-control.service.ts", [
  ...
  write(root, "scripts/hard-delete-gate.js", [
  ...
  write(root, "package.json", '{"scripts":{"purchasing:ap:gate":"node gate","policy:gates":"npm run purchasing:ap:gate"}}')
}
...
describe("purchasing AP consolidation gate", () => {
  ...
  it("blocks when posted AP journals lose close invalidation", () => {
    ...
    fs.writeFileSync(target, fs.readFileSync(target, "utf8").replace("recordPostedJournalCloseInvalidationInTx(", "closeInvalidationMissing("), "utf8")
  ...
  it("blocks when supplier invoices lose received-quantity evidence", () => {
    ...
    writeReadyFixture(root)
    const target = path.join(root, "services/purchasing/ap-control.service.ts")
    fs.writeFileSync(target, fs.readFileSync(target, "utf8").replace("Supplier invoice quantity exceeds received and uninvoiced goods.", "quantity check missing"), "utf8")
