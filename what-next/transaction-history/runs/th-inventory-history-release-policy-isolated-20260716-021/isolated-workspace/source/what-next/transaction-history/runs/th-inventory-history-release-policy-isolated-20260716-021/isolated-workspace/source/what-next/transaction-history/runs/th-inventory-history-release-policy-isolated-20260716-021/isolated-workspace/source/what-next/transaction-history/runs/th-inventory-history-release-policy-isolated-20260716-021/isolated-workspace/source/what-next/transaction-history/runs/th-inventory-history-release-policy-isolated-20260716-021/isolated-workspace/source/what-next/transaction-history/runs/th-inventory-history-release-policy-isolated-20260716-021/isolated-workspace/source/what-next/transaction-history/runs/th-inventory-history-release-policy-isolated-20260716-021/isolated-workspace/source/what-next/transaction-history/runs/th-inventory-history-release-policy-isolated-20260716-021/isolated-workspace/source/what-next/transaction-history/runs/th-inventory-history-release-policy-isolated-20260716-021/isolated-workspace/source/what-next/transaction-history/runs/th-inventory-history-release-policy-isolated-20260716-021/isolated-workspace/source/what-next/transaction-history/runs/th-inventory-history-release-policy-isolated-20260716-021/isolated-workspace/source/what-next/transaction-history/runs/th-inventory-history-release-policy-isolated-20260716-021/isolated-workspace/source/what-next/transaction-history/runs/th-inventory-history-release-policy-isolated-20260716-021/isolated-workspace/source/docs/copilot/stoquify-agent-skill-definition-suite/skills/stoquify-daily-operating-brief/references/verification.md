...
# Verification — Daily Operating Brief

## Structural

- Validate frontmatter name equals `stoquify-daily-operating-brief` and contains only `name` and `description`.
- Validate `agents/openai.yaml` and confirm the default prompt invokes `$stoquify-daily-operating-brief`.
