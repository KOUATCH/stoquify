---
name: stoquify-cross-domain-root-cause-trace
description: Use when Stoquify must trace a signal through source record, business event, workflow, ledger or evidence link, approval, and outcome without inferring missing relationships. Use for now-phase Copilot workflows requiring high-risk controls, evidence, safe failure, and bounded autonomy.
---

# Cross-Domain Root-Cause Trace

## Purpose

Trace a signal through source record, business event, workflow, ledger or evidence link, approval, and outcome without inferring missing relationships.

## Scope and non-scope

- Operate as capability `S18` in the Stoquify Copilot registry.
...
4. Read only the relevant source services from: `services/events`, `services/history`, `services/stock-to-cash`, `services/accounting`, `services/evidence`.
5. Read upstream capability evidence for: stoquify-trusted-context-resolver, stoquify-permission-entitlement-guard, stoquify-evidence-grounded-retrieval, stoquify-agent-evidence-recorder, stoquify-freshness-trust-evaluator.
...
5. Perform only this skill's bounded responsibility: Trace a signal through source record, business event, workflow, ledger or evidence link, approval, and outcome without inferring missing relationships.
...
- Permitted service domains: `events`, `history`, `stock-to-cash`, `accounting`, `evidence`.
...
Risk class: `high`. Autonomy ceiling: `read`. Default classification is `confidential`; payroll, secrets, credentials, bank destinations, identity data, and sensitive financial evidence are `restricted`. Minimize and redact before model use.

## Safe failure and stop conditions

Return PARTIAL with the broken link and next evidence request.
