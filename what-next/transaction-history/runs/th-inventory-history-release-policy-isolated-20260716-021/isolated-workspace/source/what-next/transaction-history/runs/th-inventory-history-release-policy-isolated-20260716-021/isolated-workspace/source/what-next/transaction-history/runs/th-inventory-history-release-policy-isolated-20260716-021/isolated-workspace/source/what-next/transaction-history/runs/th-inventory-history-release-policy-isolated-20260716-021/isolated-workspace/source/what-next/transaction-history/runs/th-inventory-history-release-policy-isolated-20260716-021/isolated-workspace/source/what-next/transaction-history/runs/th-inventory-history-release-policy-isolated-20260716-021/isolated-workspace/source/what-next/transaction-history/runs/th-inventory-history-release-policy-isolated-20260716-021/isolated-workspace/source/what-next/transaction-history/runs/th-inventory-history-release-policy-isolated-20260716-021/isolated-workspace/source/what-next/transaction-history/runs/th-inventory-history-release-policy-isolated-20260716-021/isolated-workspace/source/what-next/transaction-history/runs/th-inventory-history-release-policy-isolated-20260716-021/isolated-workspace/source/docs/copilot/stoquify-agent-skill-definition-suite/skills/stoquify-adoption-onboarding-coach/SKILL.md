---
name: stoquify-adoption-onboarding-coach
description: Use when Stoquify must convert consented product telemetry and versioned playbooks into role-specific setup, training, and adoption actions. Use for later-phase Copilot workflows requiring medium-risk controls, evidence, safe failure, and bounded autonomy.
---

# Adoption and Onboarding Coach

## Purpose

Convert consented product telemetry and versioned playbooks into role-specific setup, training, and adoption actions.

## Scope and non-scope

- Operate as capability `S28` in the Stoquify Copilot registry.
...
4. Read only the relevant source services from: `services/analytics`, `services/dashboard`, `services/users`, `services/organization`.
5. Read upstream capability evidence for: stoquify-trusted-context-resolver, stoquify-permission-entitlement-guard, stoquify-evidence-grounded-retrieval, stoquify-agent-evidence-recorder, stoquify-freshness-trust-evaluator, stoquify-model-cost-router, stoquify-explicit-preference-memory, stoquify-daily-operating-brief.
...
5. Perform only this skill's bounded responsibility: Convert consented product telemetry and versioned playbooks into role-specific setup, training, and adoption actions.
...
- Permitted service domains: `analytics`, `dashboard`, `users`, `organization`.
...
Return UNKNOWN until telemetry, consent, and health definitions are reliable; never guess usage or customer intent.
