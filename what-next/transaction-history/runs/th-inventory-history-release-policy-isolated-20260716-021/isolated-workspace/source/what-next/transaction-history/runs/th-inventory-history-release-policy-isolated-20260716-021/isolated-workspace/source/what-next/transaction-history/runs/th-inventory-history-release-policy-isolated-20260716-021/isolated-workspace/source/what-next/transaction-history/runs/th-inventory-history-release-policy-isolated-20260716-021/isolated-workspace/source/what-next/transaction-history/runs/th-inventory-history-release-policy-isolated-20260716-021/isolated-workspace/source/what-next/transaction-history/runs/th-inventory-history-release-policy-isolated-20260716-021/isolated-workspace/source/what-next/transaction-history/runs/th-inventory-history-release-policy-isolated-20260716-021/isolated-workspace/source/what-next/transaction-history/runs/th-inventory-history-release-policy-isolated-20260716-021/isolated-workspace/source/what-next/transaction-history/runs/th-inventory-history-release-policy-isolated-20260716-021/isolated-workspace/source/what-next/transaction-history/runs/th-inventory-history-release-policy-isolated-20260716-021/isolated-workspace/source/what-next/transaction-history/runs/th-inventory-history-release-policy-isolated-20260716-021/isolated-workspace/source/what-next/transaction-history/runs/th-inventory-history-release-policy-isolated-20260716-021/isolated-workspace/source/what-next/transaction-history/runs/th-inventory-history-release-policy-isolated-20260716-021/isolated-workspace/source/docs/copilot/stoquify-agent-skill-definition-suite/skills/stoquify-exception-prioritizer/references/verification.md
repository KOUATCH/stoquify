...
# Verification — Exception Prioritizer

## Structural

- Validate frontmatter name equals `stoquify-exception-prioritizer` and contains only `name` and `description`.
- Validate `agents/openai.yaml` and confirm the default prompt invokes `$stoquify-exception-prioritizer`.
