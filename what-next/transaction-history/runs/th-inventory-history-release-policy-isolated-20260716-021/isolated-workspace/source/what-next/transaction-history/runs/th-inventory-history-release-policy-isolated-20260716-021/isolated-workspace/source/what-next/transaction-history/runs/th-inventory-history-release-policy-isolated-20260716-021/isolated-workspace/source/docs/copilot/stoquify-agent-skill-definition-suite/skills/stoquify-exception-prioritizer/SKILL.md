---
name: stoquify-exception-prioritizer
description: Use when Stoquify must deduplicate and rank cases by severity, value exposure, deadline, evidence quality, close impact, and accountable role without inventing urgency. Use for now-phase Copilot workflows requiring medium-risk controls, evidence, safe failure, and bounded autonomy.
---
...
Deduplicate and rank cases by severity, value exposure, deadline, evidence quality, close impact, and accountable role without inventing urgency.

## Scope and non-scope

- Operate as capability `S10` in the Stoquify Copilot registry.
...
4. Read only the relevant source services from: `services/signals`, `services/manager-action-center`, `services/assurance`.
5. Read upstream capability evidence for: stoquify-trusted-context-resolver, stoquify-evidence-grounded-retrieval, stoquify-freshness-trust-evaluator.
...
5. Perform only this skill's bounded responsibility: Deduplicate and rank cases by severity, value exposure, deadline, evidence quality, close impact, and accountable role without inventing urgency.
...
- Permitted service domains: `signals`, `manager-action-center`, `assurance`.
...
Return an unranked review queue when scoring evidence is insufficient.
