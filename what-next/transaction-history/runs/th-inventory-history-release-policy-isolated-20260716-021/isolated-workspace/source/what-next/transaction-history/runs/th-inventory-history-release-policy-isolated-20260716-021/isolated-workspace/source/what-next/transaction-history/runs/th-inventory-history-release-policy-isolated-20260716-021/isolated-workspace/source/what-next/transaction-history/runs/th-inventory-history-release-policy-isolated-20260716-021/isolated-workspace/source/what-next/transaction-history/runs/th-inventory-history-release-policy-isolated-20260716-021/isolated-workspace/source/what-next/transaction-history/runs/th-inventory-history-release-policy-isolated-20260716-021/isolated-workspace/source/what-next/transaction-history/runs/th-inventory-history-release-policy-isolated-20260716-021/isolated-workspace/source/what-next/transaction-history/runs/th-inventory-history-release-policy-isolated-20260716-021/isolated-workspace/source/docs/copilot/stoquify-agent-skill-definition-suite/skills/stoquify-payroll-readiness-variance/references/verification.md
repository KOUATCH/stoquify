...
# Verification — Payroll Readiness and Variance

## Structural

- Validate frontmatter name equals `stoquify-payroll-readiness-variance` and contains only `name` and `description`.
- Validate `agents/openai.yaml` and confirm the default prompt invokes `$stoquify-payroll-readiness-variance`.
