...
export function ReportTrustBanner({ provenance }: ReportTrustBannerProps) {
  if (!provenance) {
    return (
      <div className="rounded-md border border-[var(--dash-warning)] bg-[var(--dash-warning-soft)] px-4 py-3 text-sm text-[var(--dash-warning)]">
      ...
      <div className="flex flex-col gap-3 lg:flex-row lg:items-start lg:justify-between">
        <div className="space-y-1">
          <div className="flex flex-wrap items-center gap-2">
            <Badge variant="secondary">{provenance.sourceLabel}</Badge>
            <Badge variant="outline">{freshnessLabel(provenance.freshness)}</Badge>
            <Badge variant="outline">{provenance.certificationLabel}</Badge>
          </div>
