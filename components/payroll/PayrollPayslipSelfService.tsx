...
function statusTone(value: string) {
  ...
  if (["EMITTED", "READY", "SETTLED_OR_RELEASED", "POSTED", "SUPPORTED"].includes(normalized)) {
    return "border-emerald-400/30 bg-emerald-400/10 text-emerald-100"
  }
  if (["UNPAID", "IN_PROGRESS", "PARTIALLY_SUPPORTED", "NOT_SUPPORTED", "NOT_YET_SUPPORTED"].includes(normalized)) {
...
export default function PayrollPayslipSelfService({ data, error, locale }: Props) {
  if (error) {
    return (
  ...
  return (
    <main className="flex min-w-0 flex-col gap-4 text-slate-100">
      <section className="flex flex-col gap-3 border-b border-white/10 pb-4 xl:flex-row xl:items-end xl:justify-between">
        <div className="min-w-0">
          <p className="text-xs font-semibold uppercase tracking-normal text-cyan-200">Employee self-service</p>
          <h1 className="mt-1 text-2xl font-bold text-white sm:text-3xl">My payslips</h1>
      ...
      <section className="grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
        <Metric label="Payslips" value={data.summary.payslipCount} icon={FileText} />
        <Metric label="Latest issue" value={dateLabel(data.summary.latestIssuedAt)} icon={CalendarDays} />
        <Metric label="Payment evidence" value={data.employee.paymentDestinationApproved ? "Approved" : "Pending"} icon={Banknote} />
      ...
      {data.payslips.length ? (
        <section className="flex flex-col gap-4">
          {data.payslips.map((payslip) => (
            <article key={payslip.id} className="rounded-lg border border-white/10 bg-white/[0.05]">
              <div className="grid gap-3 border-b border-white/10 px-4 py-4 lg:grid-cols-[minmax(0,1fr)_auto] lg:items-start">
                <div className="min-w-0">
                  <div className="flex min-w-0 flex-wrap items-center gap-2">
                    <h2 className="truncate text-base font-semibold text-white">{payslip.payslipNumber}</h2>
                ...
                <div className="min-w-0 text-right">
                  <p className="text-xs font-semibold uppercase tracking-normal text-slate-400">Net payable</p>
                  <p className="mt-1 break-words text-xl font-bold text-white">
                    {money(payslip.amounts.netPayableAmount, payslip.amounts.currency)}
                  </p>
                </div>
              </div>
                ...
                <div className="min-w-0 rounded-lg border border-white/10 bg-white/[0.04]">
                  ...
                  <div className="divide-y divide-white/10">
                    {payslip.lines.map((line) => (
                      <div key={line.id} className="grid gap-2 px-4 py-3 sm:grid-cols-[minmax(0,1fr)_auto]">
                        <div className="min-w-0">
                          <p className="truncate text-sm font-semibold text-white">{line.label}</p>
                          <p className="mt-1 truncate text-xs text-slate-500">{line.code} / {line.category}</p>
                        </div>
                        <p className="text-sm font-semibold text-slate-100">{money(line.amount, line.currency)}</p>
                      </div>
                    ))}
                  </div>
                </div>
                ...
                <div className="min-w-0 rounded-lg border border-white/10 bg-white/[0.04]">
                  ...
                  <dl className="divide-y divide-white/10">
                    <div className="grid gap-2 px-4 py-3 sm:grid-cols-[170px_minmax(0,1fr)]">
                      <dt className="text-xs font-semibold uppercase tracking-normal text-slate-400">Document hash</dt>
                      <dd className="min-w-0 break-all text-sm text-slate-100">{payslip.proof.documentHash}</dd>
                    </div>
                    <div className="grid gap-2 px-4 py-3 sm:grid-cols-[170px_minmax(0,1fr)]">
                      <dt className="text-xs font-semibold uppercase tracking-normal text-slate-400">Archive hash</dt>
                      <dd className="min-w-0 break-all text-sm text-slate-100">{payslip.proof.archiveManifestHash}</dd>
                    </div>
                    <div className="grid gap-2 px-4 py-3 sm:grid-cols-[170px_minmax(0,1fr)]">
                      <dt className="text-xs font-semibold uppercase tracking-normal text-slate-400">Ledger batch</dt>
                      <dd className="min-w-0 break-all text-sm text-slate-100">{payslip.tieOut.ledgerPostingBatchId ?? "Pending"}</dd>
                    </div>
                    <div className="grid gap-2 px-4 py-3 sm:grid-cols-[170px_minmax(0,1fr)]">
                      <dt className="text-xs font-semibold uppercase tracking-normal text-slate-400">Country scope</dt>
        ...
        <EmptyState title="Payslips" body="No emitted payslips were found for this employee profile." />
      )}
    </main>
  )
}
