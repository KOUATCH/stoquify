...
export function LandingHero() {
  ...
  const metrics = [
    ["branches", t("metrics.branches.value"), t("metrics.branches.label")],
    ["accuracy", t("metrics.accuracy.value"), t("metrics.accuracy.label")],
    ["close", t("metrics.close.value"), t("metrics.close.label")],
  ]

  return (
    <section className="relative isolate overflow-hidden bg-[var(--color-canvas)] px-6 pb-14 pt-12 text-[var(--color-text-primary)] sm:pb-16 sm:pt-14 lg:px-8 lg:pb-8 lg:pt-14">
      <div className="landing-grid-bg absolute inset-0" />
      <div className="landing-grain" />
      <div className="absolute inset-x-0 top-0 h-px bg-gradient-to-r from-transparent via-[var(--color-spruce)]/45 to-[var(--color-brand)]/45" />
        ...
        <div className="min-w-0">
          ...
          <div className="mt-10 grid max-w-2xl grid-cols-3 gap-3">
            {metrics.map(([key, value, label]) => (
              <div key={key} className="rounded-lg border border-[var(--color-border-subtle)] bg-white/[0.045] p-3 sm:p-4">
                <div className="data-text text-xl font-semibold text-[var(--color-text-primary)] sm:text-2xl">{value}</div>
                <div className="mt-1 body-text text-[0.68rem] text-[var(--color-text-tertiary)] sm:text-xs">{label}</div>
              </div>
            ))}
          </div>
          <div className="mt-10 hidden overflow-hidden rounded-lg border border-[var(--color-border-subtle)] bg-black/10 py-3 sm:block">
