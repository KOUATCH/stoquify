---
name: priority-001-green-baseline-ratchets
description: "Priority 001 Green Baseline Ratchets remediates an AqStoqFlow priority implementation class. Use when executing the ordered priority remediation suite, hardening this codebase priority, migrating legacy code into service-owned workflows, adding OHADA controls, or validating this release gate."
---

# Green Baseline Ratchets
...
Risk: Without a stable baseline, later remediation can mix real regressions with pre-existing noise and make release hardening unreliable.
...
1. Read the latest enterprise examination, statutory scan, certification scan, and service-boundary report.
2. Capture the currently passing verification commands and known non-blocking warnings.
3. Repair only broken verification scripts or missing report-mode gates needed to preserve the baseline.
4. Do not refactor business modules in this skill unless a baseline command cannot run without a surgical repair.
5. Save a baseline report under what-next with commands, results, warnings, and next eligible priority skill.
...
Save a report under `what-next/` named `AQSTOQFLOW_PRIORITY_001_GREEN_BASELINE_RATCHETS_REPORT_2026-06-16.md` containing:
