...
| `/en/dashboard/settings/data-onboarding` | Governed master-data onboarding | `app/[locale]/(dashboard)/dashboard/settings/data-onboarding/*`, `components/onboarding/MasterDataOnboardingWorkbench.tsx` | `aligned` | Labelled native controls, visible focus, live success/error announcements, non-color risk/status text, uploader-blocked high-risk approval, and localized EN/FR loading, empty, partial, permission, module-lock, error, and recovery copy. | Tokenized workbench uses overflow-safe hashes, responsive grids, and desktop/mobile browser targets. | Tier 1 for the first release-bound workflow: EN/FR desktop/mobile Axe, overflow, keyboard, and screenshot evidence. | Run `npm run test:e2e:master-data-onboarding` after the isolated test database has only reviewed migrations pending; attach `what-next/evidence/master-data-onboarding-2026-08-16/browser-matrix.json`. |
