...
import { LandingSectionNavigation } from "@/components/landing/landing-section-navigation"
...
export function LandingHeader({ session }: LandingHeaderProps) {
  ...
  return (
    <header data-landing-header className="sticky top-0 z-50 border-b border-[var(--color-border-subtle)] bg-[var(--color-chrome)]/92 text-[var(--color-text-primary)] backdrop-blur-xl">
      <div className="mx-auto flex h-16 max-w-7xl items-center justify-between px-6 lg:px-8">
        <Link href="/" className="group inline-flex items-center gap-3" aria-label={t("homeAria")}>
          <StoquifyLogo theme="dark" width={212} height={48} tagline={t("subtitle")} />
        </Link>

        <LandingSectionNavigation variant="desktop" />
        ...
        <div className="hidden items-center gap-3 xl:flex">
          ...
          {session ? (
            ...
            <>
              ...
              <Link href="/register" className="inline-flex items-center gap-2 rounded-lg bg-[var(--color-brand-action)] px-4 py-2 body-text text-sm font-semibold text-white transition hover:bg-[var(--color-brand-hover)]">
                {t("startFree")}
        ...
        <details data-landing-mobile-menu className="group relative xl:hidden">
          ...
          <div className="absolute right-0 mt-3 w-64 rounded-lg border border-[var(--color-border-subtle)] bg-[var(--color-surface-panel)] p-3 shadow-2xl shadow-black/30">
            <div className="grid gap-1">
              ...
              {!session ? (
                <Link href="/register" className="mt-2 rounded-lg bg-[var(--color-brand-action)] px-3 py-2 text-center body-text text-sm font-semibold text-white">
                  {t("startFree")}
                </Link>
              ) : null}
            </div>
