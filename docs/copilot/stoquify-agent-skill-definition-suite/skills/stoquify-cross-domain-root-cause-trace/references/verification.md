...
# Verification — Cross-Domain Root-Cause Trace

## Structural

- Validate frontmatter name equals `stoquify-cross-domain-root-cause-trace` and contains only `name` and `description`.
- Validate `agents/openai.yaml` and confirm the default prompt invokes `$stoquify-cross-domain-root-cause-trace`.
