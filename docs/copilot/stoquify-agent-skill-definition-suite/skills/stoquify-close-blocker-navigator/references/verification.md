...
# Verification — Close Blocker Navigator

## Structural

- Validate frontmatter name equals `stoquify-close-blocker-navigator` and contains only `name` and `description`.
- Validate `agents/openai.yaml` and confirm the default prompt invokes `$stoquify-close-blocker-navigator`.
