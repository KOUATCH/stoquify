...
# Verification — Agent Run State Machine

## Structural

- Validate frontmatter name equals `stoquify-agent-run-state-machine` and contains only `name` and `description`.
- Validate `agents/openai.yaml` and confirm the default prompt invokes `$stoquify-agent-run-state-machine`.
