...
const DASHBOARD_SESSION_RECOVERY_MESSAGE =
  'Your dashboard session points to an organization that is no longer available. Please sign out and sign back in.'

function isUsableOrganization(
  organization?: { id: string; deletedAt: Date | null; isActive: boolean } | null
): organization is { id: string; deletedAt: Date | null; isActive: boolean } {
  return !!organization && organization.deletedAt === null && organization.isActive
}

async function resolveDashboardOrganizationId(requestedOrganizationId: string) {
  ...
  const [freshUser, requestedOrganization] = await Promise.all([
    user.id
      ? db.user.findUnique({
          where: { id: user.id },
          select: {
  ...
  const effectiveUserOrganizationId = freshUser?.organizationId || user.organizationId
  const canUseRequestedOrganization =
    requestedOrganization &&
    isUsableOrganization(requestedOrganization) &&
    (permissions.includes('*') ||
      effectiveUserOrganizationId === requestedOrganization.id ||
      user.organizationId === requestedOrganization.id)

  if (canUseRequestedOrganization) {
    return requestedOrganization.id
  }

  if (isUsableOrganization(freshUser?.organization)) {
    return freshUser.organization.id
  }
  ...
  if (requestedOrganization && !permissions.includes('*')) {
    throw new Error('You do not have access to this organization dashboard.')
  }

  throw new Error(DASHBOARD_SESSION_RECOVERY_MESSAGE)
}
...
export async function getAllDashboardData(
  ...
  const organizationId = await resolveDashboardOrganizationId(requestedOrganizationId)
...
export async function getDashboardMetrics(organizationId: string): Promise<LegacyDashboardMetrics> {
  const data = await getAllDashboardData(organizationId)
