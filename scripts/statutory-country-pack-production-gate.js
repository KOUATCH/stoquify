...
function sourceEvidenceState(root, countryPackSource) {
  ...
  if (!loaded || !Array.isArray(loaded.manifest.artifacts)) {
    return {
      ...
      declaredSourceHashCount: 0,
      validDeclaredSourceHashCount: 0,
      boundDeclaredSourceHashCount: 0,
      hashesVerified: false,
      approvalArtifactVerified: false,
      expertApprovalComplete: false,
    };
  }
  ...
  const validDeclaredHashes = declaredHashes.filter((hash) =>
    /^sha256:[a-f0-9]{64}$/.test(hash),
  );
  const boundDeclaredHashes = validDeclaredHashes.filter((hash) =>
    artifactHashes.has(hash),
  );
  const hashesVerified =
    artifactsVerified &&
    declaredHashes.length > 0 &&
    boundDeclaredHashes.length === declaredHashes.length;
  ...
  const approvalArtifactVerified = Boolean(
    approval &&
    typeof approval.reviewerIdentity === "string" &&
    approval.reviewerIdentity.trim() &&
    typeof approval.reviewedAt === "string" &&
    approval.reviewedAt.trim() &&
    typeof approval.effectiveFrom === "string" &&
    approval.effectiveFrom.trim() &&
    typeof approval.approvalArtifactFile === "string" &&
    path.basename(approval.approvalArtifactFile) ===
      approval.approvalArtifactFile &&
    typeof approval.approvalArtifactHash === "string" &&
    /^[a-f0-9]{64}$/.test(approval.approvalArtifactHash) &&
    Array.isArray(approval.approvedFixtureFamilies) &&
    approval.approvedFixtureFamilies.length === requiredFixtureFamilies.size &&
    approval.approvedFixtureFamilies.every((family) =>
      requiredFixtureFamilies.has(family),
    ) &&
    fs.existsSync(path.join(manifestDir, approval.approvalArtifactFile)) &&
    crypto
      .createHash("sha256")
      .update(
        fs.readFileSync(path.join(manifestDir, approval.approvalArtifactFile)),
      )
      .digest("hex") === approval.approvalArtifactHash,
  );
  const expertApprovalComplete =
    hashesVerified &&
    loaded.manifest.productionUseAllowed === true &&
    approvedStatuses.has(loaded.manifest.reviewStatus) &&
    loaded.manifest.artifacts.every(
      (artifact) =>
        artifact.productionUseAllowed === true &&
        approvedStatuses.has(artifact.reviewStatus),
    ) &&
  ...
  return {
    ...
    artifactHashesVerified: artifactsVerified,
    declaredSourceHashCount: declaredHashes.length,
    validDeclaredSourceHashCount: validDeclaredHashes.length,
    boundDeclaredSourceHashCount: boundDeclaredHashes.length,
    hashesVerified,
    approvalArtifactVerified,
    expertApprovalComplete,
  };
}

function runtimeAuthorityState(countryPackSource) {
  const capabilityStatus =
    countryPackSource.match(
      /const\s+CNPS_CAPABILITY_STATUS\s*=\s*["']([^"']+)["']\s+as\s+const/,
    )?.[1] || null;
  const verificationStatus =
    countryPackSource.match(
      /const\s+CNPS_VERIFICATION_STATUS\s*=\s*["']([^"']+)["']\s+as\s+const/,
    )?.[1] || null;
  const authoritativeCapabilities = new Set([
    "SUPPORTED",
    "SUPPORTED_CERTIFIED",
  ]);
  const authoritativeVerification = new Set([
    "EXPERT_REVIEWED",
    "REGULATOR_CONFIRMED",
  ]);
  return {
    ...
    verificationStatus,
    authoritative:
      authoritativeCapabilities.has(capabilityStatus) &&
      authoritativeVerification.has(verificationStatus),
  };
}
function buildStatutoryCountryPackReadiness(
  root = process.cwd(),
  options = {},
) {
  ...
  const runtimeAuthority = runtimeAuthorityState(cameroon);

  const checks = [
    {
      id: "country_pack_provenance_schema",
      ready:
        schemas.includes("countryPackHeaderSchema") &&
        schemas.includes("legalReferenceSchema") &&
        schemas.includes("effectiveFrom") &&
        schemas.includes("verifiedBy") &&
        schemas.includes("hash: z.string().regex(/^sha256:"),
    },
    {
      id: "published_effective_resolution",
      ready:
        resolve.includes('pack.header.status === "PUBLISHED"') &&
        resolve.includes("isDateInPackWindow") &&
        resolve.includes("pinnedPackVersion") &&
        resolve.includes("resolutionHash"),
    },
    {
      id: "publish_requires_reviewed_evidence",
      ready:
        validation.includes("validateCountryPackForPublish") &&
        validation.includes("requirePublished: true") &&
        validation.includes("requireNoExpertReview: true") &&
        validation.includes("GOLDEN_FIXTURE_FAILED"),
    },
    {
    ...
    },
    {
      ...
      ready:
        sourceEvidence.expertApprovalComplete && runtimeAuthority.authoritative,
    },
    {
    ...
    },
    {
      ...
      ready:
        payrollEvaluator.includes("PRODUCTION_TAX_CAPABILITY_STATUSES") &&
        payrollEvaluator.includes(
    ...
    },
    {
      ...
      ready:
        registry.includes("fakeSandboxComplianceAdapter") &&
        registry.includes("cameroonDgiSandboxComplianceAdapter") &&
        registry.includes(
          "official specifications, sandbox proof, and expert review",
        ) &&
    ...
    },
    {
      ...
      ready:
        outbox.includes(
          "parsed.environment === ComplianceAdapterEnvironment.PRODUCTION",
        ) &&
        outbox.includes(
    ...
    },
    {
      ...
      ready:
        hardcodeGate.includes("regulatory-hardcode") &&
        packageJson.includes('"regulatory:hardcode:fail"') &&
        packageJson.includes('"statutory:country-pack:gate"') &&
        packageJson.includes("npm run statutory:country-pack:gate"),
    },
  ];
...
function renderMarkdown(report) {
  const lines = [
    ...
    "",
    "- Manifest: " + (report.sourceEvidence.manifestPath || "missing"),
    "- Captured artifact hashes verified: " +
      report.sourceEvidence.verifiedArtifactCount +
      "/" +
      report.sourceEvidence.artifactCount,
    "- Pack source hashes declared / valid / bound: " +
      report.sourceEvidence.declaredSourceHashCount +
      "/" +
      report.sourceEvidence.validDeclaredSourceHashCount +
      "/" +
      report.sourceEvidence.boundDeclaredSourceHashCount,
    "- Approval artifact verified: " +
      report.sourceEvidence.approvalArtifactVerified,
    "- Qualified expert approval complete: " +
      report.sourceEvidence.expertApprovalComplete,
    "- Runtime CNPS capability status: " +
      (report.runtimeAuthority.capabilityStatus || "missing"),
    "- Runtime CNPS verification status: " +
      (report.runtimeAuthority.verificationStatus || "missing"),
    "- Runtime CNPS authority binding promoted: " +
      report.runtimeAuthority.authoritative,
    "",
    "## Safety",
    "",
