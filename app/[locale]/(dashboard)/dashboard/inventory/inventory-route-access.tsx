...
import type {
  InventoryRouteSurface,
  InventoryRouteSurfaceModule,
  PermissionMode,
} from "./inventory-route-data-access"
import { getInventoryRouteSurface } from "./inventory-route-data-access"
...
type InventoryRouteContext = RbacContext

type InventoryRouteAccessResult =
  | {
      kind: "blocked"
      node: ReactNode
    }
...
const NO_ACTIVE_ORG_MESSAGE =
  "Refresh your session from the dashboard so this route can load tenant-scoped inventory data."
...
function getModuleRequirements(surface: InventoryRouteSurface): InventoryRouteSurfaceModule[] {
  if (surface.modules && surface.modules.length > 0) return surface.modules
  return surface.module ? [surface.module] : []
}

async function evaluateInventoryRouteAccess(
  surface: InventoryRouteSurface,
  localePromise: Promise<{ locale: string }>,
  permissionOptions: PermissionInput = {},
): Promise<InventoryRouteAccessResult> {
  const { locale: rawLocale } = await localePromise
  const locale = pickLocale(rawLocale)

  let context: InventoryRouteContext
...
export async function withInventorySurfaceAccess({
  params,
  surface,
  permissionOptions,
  onAllowed,
  ...
  surface: InventoryRouteSurface
  permissionOptions?: PermissionInput
  onAllowed: (context: InventoryRouteContext, locale: Locale) => Promise<ReactNode> | ReactNode
}) {
  const state = await evaluateInventoryRouteAccess(surface, params, permissionOptions)
...
export function routeByKey(key: string) {
  return getInventoryRouteSurface(key)
}
