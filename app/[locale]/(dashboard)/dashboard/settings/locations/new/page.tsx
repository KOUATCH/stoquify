...
async function NewLocationPageImpl({
  params,
}: {
...
export default async function SettingsRoutePage(props: any = {}) {
  const surface = routeByKey("settings-locations-new")

  if (!surface) {
    throw new Error("Missing settings route surface definition: settings-locations-new")
  }

  return withSettingsSurfaceAccess({
    params: (props as { params?: Promise<{ locale: string }> }).params ?? Promise.resolve({ locale: "en" }),
    surface,
    onAllowed: async () => NewLocationPageImpl(props),
  })
}
