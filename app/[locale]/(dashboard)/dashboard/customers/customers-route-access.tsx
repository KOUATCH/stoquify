...
import type {
  CustomersRouteSurface,
  CustomersRouteSurfaceModule,
  PermissionMode,
} from "./customers-route-data-access"
import { getCustomersRouteSurface } from "./customers-route-data-access"
...
type CustomersRouteContext = RbacContext

type CustomersRouteAccessResult =
  | {
      kind: "blocked"
      node: ReactNode
    }
...
const NO_ACTIVE_ORG_MESSAGE =
  "Refresh your session from the dashboard so this route can load tenant-scoped customer data."
...
function getModuleRequirements(surface: CustomersRouteSurface): CustomersRouteSurfaceModule[] {
  if (surface.modules && surface.modules.length > 0) return surface.modules
  return surface.module ? [surface.module] : []
}

async function evaluateCustomersRouteAccess(
  surface: CustomersRouteSurface,
  localePromise: Promise<{ locale: string }>,
  permissionOptions: PermissionInput = {},
): Promise<CustomersRouteAccessResult> {
  const { locale: rawLocale } = await localePromise
  const locale = pickLocale(rawLocale)

  let context: CustomersRouteContext
  ...
  try {
    const mode: PermissionMode = surface.permissionMode ?? "all"
...
export async function withCustomersSurfaceAccess({
  params,
  surface,
  permissionOptions,
  onAllowed,
  ...
  surface: CustomersRouteSurface
  permissionOptions?: PermissionInput
  onAllowed: (context: CustomersRouteContext, locale: Locale) => Promise<ReactNode> | ReactNode
}) {
  const state = await evaluateCustomersRouteAccess(surface, params, permissionOptions)
...
export function routeByKey(key: string) {
  return getCustomersRouteSurface(key)
}
