...
import type {
  AccountingRouteSurface,
  AccountingRouteSurfaceModule,
  PermissionMode,
} from "./accounting-route-data-access"
import { getAccountingRouteSurface } from "./accounting-route-data-access"
...
type AccountingRouteContext = RbacContext

type AccountingRouteAccessResult =
  | {
      kind: "blocked"
      node: ReactNode
    }
  | {
      kind: "allowed"
      context: AccountingRouteContext
      locale: Locale
    }
...
const NO_ACTIVE_ORG_MESSAGE =
  "Refresh your session from the dashboard so this route can load tenant-scoped accounting data."
...
function getModuleRequirements(surface: AccountingRouteSurface): AccountingRouteSurfaceModule[] {
  if (surface.modules && surface.modules.length > 0) return surface.modules
  return surface.module ? [surface.module] : []
}

async function evaluateAccountingRouteAccess(
  surface: AccountingRouteSurface,
  localePromise: Promise<{ locale: string }>,
  permissionOptions: PermissionInput = {},
): Promise<AccountingRouteAccessResult> {
  const { locale: rawLocale } = await localePromise
  const locale = pickLocale(rawLocale)

  let context: AccountingRouteContext
...
export async function withAccountingSurfaceAccess({
  params,
  surface,
  permissionOptions,
  onAllowed,
  ...
  surface: AccountingRouteSurface
  permissionOptions?: PermissionInput
  onAllowed: (context: AccountingRouteContext, locale: Locale) => Promise<ReactNode> | ReactNode
}) {
  const state = await evaluateAccountingRouteAccess(surface, params, permissionOptions)
...
export function routeByKey(key: string) {
  return getAccountingRouteSurface(key)
}
