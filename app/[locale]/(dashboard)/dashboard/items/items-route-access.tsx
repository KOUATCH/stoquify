...
import type { ItemsRouteSurface, PermissionMode } from "./items-route-data-access"
import { getItemsRouteSurface } from "./items-route-data-access"

type ItemsRouteContext = Awaited<ReturnType<typeof requirePermission>>
...
function getPermissionMode(surface: ItemsRouteSurface): PermissionMode {
  return surface.permissionMode ?? "single"
}

async function evaluateItemsRouteAccess(
  surface: ItemsRouteSurface,
  localePromise: Promise<{ locale: string }>,
  permissionOptions: PermissionInput = {},
): Promise<ItemsRouteAccessResult> {
  const { locale: rawLocale } = await localePromise
  const locale = pickLocale(rawLocale)
  let context: ItemsRouteContext
...
export async function withItemsSurfaceAccess({
  params,
  surface,
  permissionOptions,
  onAllowed,
  ...
  surface: ItemsRouteSurface
  permissionOptions?: PermissionInput
  onAllowed: (context: ItemsRouteContext, locale: Locale) => Promise<ReactNode> | ReactNode
}) {
  const state = await evaluateItemsRouteAccess(surface, params, permissionOptions)
...
export function routeByKey(key: string) {
  return getItemsRouteSurface(key)
}
